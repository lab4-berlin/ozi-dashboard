...
def etl_load_stats_1d(context, config: Stats1dConfig):    
    ...
    context.log.info(f"Running ETL load_stats_1d for {config.iso2} from {config.date_from} to {config.date_to}.")    
    ...
    if stats:
        ...
        context.log.info(f"Successfully loaded {len(stats)} rows for {config.iso2} from {config.date_from} to {config.date_to}.")
    else:
