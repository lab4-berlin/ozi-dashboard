...
def get_last_date_from_db():
    ...
    """
    try:
        # Import the shared engine from load_to_database
        sys.path.append("/app/etl")
        from load_to_database import ENGINE
        ...
        print(
            f"ERROR: Database connection or query failed in get_last_date_from_db: {e}",
            file=sys.stderr,
        )
        return None
...
def main():
    parser = argparse.ArgumentParser(
        description="Determine date range for STATS_1D ETL job"
    )
    parser.add_argument(
        "--start-date",
