...
class TestLoadToDatabase(unittest.TestCase):
        ...
        for i in range(max_retries):
            try:
                with cls.engine.connect() as connection:
                    ...
                    print(
                        f"Schema 'data' and table 'data.asn' accessible after {i+1} attempts."
                    )
                    break
            except Exception as e:
                print(
                    f"Database connection failed: {e}, retrying in {retry_delay_seconds} seconds... (Attempt {i+1}/{max_retries})"
                )
            time.sleep(retry_delay_seconds)
        else:
            raise Exception(
                "Could not connect to database or find 'data' schema after multiple retries."
            )
    ...
    def setUp(self):
        ...
        with self.engine.connect() as connection:
            ...
            connection.execute(
                text("TRUNCATE TABLE data.country_internet_quality CASCADE;")
            )
            connection.commit()
    ...
    def test_insert_country_asns_to_db_no_duplicates(self):
        ...
        with self.engine.connect() as connection:
            ...
            # Verify the content
            asns_in_db = connection.execute(
                text(
                    "SELECT a_ripe_id, a_date, a_is_routed FROM data.asn ORDER BY a_ripe_id;"
                )
            ).fetchall()
            expected_asns = sorted(
                [
                    (
                        item["asn"],
            ...
            asns_in_db_dates_converted = [
                (asn, date.date() if isinstance(date, datetime) else date, is_routed)
                for asn, date, is_routed in asns_in_db
            ]
    ...
    def test_insert_country_stats_to_db_no_duplicates(self):
        ...
        stats = [
            {
                "timeline": [{"starttime": "2023-01-01T00:00:00Z"}],
                "v4_prefixes_ris": 10,
                "v6_prefixes_ris": 5,
                "asns_ris": 2,
                "v4_prefixes_stats": 100,
                "v6_prefixes_stats": 50,
                "asns_stats": 20,
            },
            {
                "timeline": [{"starttime": "2023-01-02T00:00:00Z"}],
                "v4_prefixes_ris": 11,
                "v6_prefixes_ris": 6,
                "asns_ris": 3,
                "v4_prefixes_stats": 101,
                "v6_prefixes_stats": 51,
                "asns_stats": 21,
            },
        ]
        ...
        insert_country_stats_to_db(
            country_iso2, resolution, stats
        )  # Attempt to insert duplicates

        with self.engine.connect() as connection:
            result = connection.execute(
                text("SELECT COUNT(*) FROM data.country_stat;")
            ).scalar()
            self.assertEqual(result, len(stats))

            # Verify content (simplified check)
            stats_in_db = connection.execute(
                text(
                    "SELECT cs_stats_timestamp FROM data.country_stat ORDER BY cs_stats_timestamp;"
                )
            ).fetchall()
            expected_timestamps = sorted(
                [
                    datetime.strptime(
                        item["timeline"][0]["starttime"], "%Y-%m-%dT%H:%M:%SZ"
                    ).replace(tzinfo=None)
                    for item in stats
                ]
            )
    ...
    def test_insert_country_asn_neighbours_to_db_no_duplicates(self):
        ...
        insert_country_asn_neighbours_to_db(
            country_iso2, neighbours
        )  # Attempt to insert duplicates

        with self.engine.connect() as connection:
            result = connection.execute(
                text("SELECT COUNT(*) FROM data.asn_neighbour;")
            ).scalar()
            self.assertEqual(result, len(neighbours))

            # Verify content (simplified check)
            neighbours_in_db = connection.execute(
                text(
                    "SELECT an_asn, an_neighbour, an_date, an_type FROM data.asn_neighbour ORDER BY an_asn, an_neighbour;"
                )
            ).fetchall()
            ...
            # Convert datetime.datetime to datetime.date for comparison
            neighbours_in_db_dates_converted = [
                (
                    asn_req,
                    asn,
                ...
                for asn_req, asn, date, type in neighbours_in_db
            ]
    ...
    def test_insert_traffic_for_country_to_db_no_duplicates(self):
        ...
        insert_traffic_for_country_to_db(
            country_iso2, traffic
        )  # Attempt to insert duplicates

        with self.engine.connect() as connection:
            result = connection.execute(
                text("SELECT COUNT(*) FROM data.country_traffic;")
            ).scalar()
            self.assertEqual(result, len(traffic["timestamps"]))

            # Verify content (simplified check)
            traffic_in_db = connection.execute(
                text(
                    "SELECT cr_date, cr_traffic FROM data.country_traffic ORDER BY cr_date;"
                )
            ).fetchall()
            expected_traffic = sorted(
                [
                    (
                    ...
                    for ts, val in zip(traffic["timestamps"], traffic["values"])
                ]
            )
            # Convert datetime.datetime to remove timezone for comparison
            traffic_in_db_converted = [
                (ts.replace(tzinfo=None) if isinstance(ts, datetime) else ts, val)
                for ts, val in traffic_in_db
            ]
    ...
    def test_insert_internet_quality_for_country_to_db_no_duplicates(self):
        ...
        insert_internet_quality_for_country_to_db(
            country_iso2, internet_quality
        )  # Attempt to insert duplicates

        with self.engine.connect() as connection:
            result = connection.execute(
                text("SELECT COUNT(*) FROM data.country_internet_quality;")
            ).scalar()
            self.assertEqual(result, len(internet_quality["timestamps"]))

            # Verify content (simplified check)
            quality_in_db = connection.execute(
                text(
                    "SELECT ci_date, ci_p75, ci_p50, ci_p25 FROM data.country_internet_quality ORDER BY ci_date;"
                )
            ).fetchall()
            ...
            # Convert datetime.datetime to remove timezone for comparison
            quality_in_db_converted = [
                (
                    ts.replace(tzinfo=None) if isinstance(ts, datetime) else ts,
                    p75,
                    p50,
                    p25,
                )
                for ts, p75, p50, p25 in quality_in_db
            ]
