...
def insert_country_asns_to_db(country_iso2, list_of_asns, save_sql_to_file=False, load_to_database=True):
    ...
    existing_asns_result = c.execute(existing_asns_query, {'country_iso2': country_iso2}).fetchall()
    existing_asns_set = set((asn, date.strftime('%Y-%m-%d')) for asn, date in existing_asns_result)

    # Filter out ASNs that already exist
    new_asns_to_insert = [
        item for item in list_of_asns
    ...
    for item in new_asns_to_insert:
        values_list.append(f"('{country_iso2}', '{item['date']}', {item['asn']}, {item['is_routed']})")
    sql += ",\n".join(values_list) + ";\n"
...
def insert_country_stats_to_db(country_iso2, resolution, stats, save_sql_to_file=False, load_to_database=True):
    ...
    existing_stats_result = c.execute(existing_stats_query, {'country_iso2': country_iso2, 'resolution': resolution}).fetchall()
    existing_stats_set = set(timestamp.strftime('%Y-%m-%d %H:%M:%S+00:00') for timestamp, in existing_stats_result)
    ...
    for item in stats:
        ...
        try:
            ...
            item_timestamp_dt = datetime.strptime(timestamp.replace('Z', ''), '%Y-%m-%dT%H:%M:%S')
        
        item_timestamp_formatted = item_timestamp_dt.strftime('%Y-%m-%d %H:%M:%S+00:00')
    ...
    sql = ("INSERT INTO data.country_stat(cs_country_iso2, cs_stats_timestamp, cs_stats_resolution, cs_v4_prefixes_ris," " cs_v6_prefixes_ris, cs_asns_ris, cs_v4_prefixes_stats, cs_v6_prefixes_stats, cs_asns_stats )\nVALUES ")
    values_list = []
    for item in new_stats_to_insert:
        v4_ris = item['v4_prefixes_ris'] if item['v4_prefixes_ris'] is not None else 'NULL'
        v6_ris = item['v6_prefixes_ris'] if item['v6_prefixes_ris'] is not None else 'NULL'
        asns_ris = item['asns_ris'] if item['asns_ris'] is not None else 'NULL'
        v4_stats = item['v4_prefixes_stats'] if item['v4_prefixes_stats'] is not None else 'NULL'
        v6_stats = item['v6_prefixes_stats'] if item['v6_prefixes_stats'] is not None else 'NULL'
...
def insert_country_asn_neighbours_to_db(country_iso2, neighbours, save_sql_to_file=False, load_to_database=True):
    ...
    existing_neighbours_query = text(f"""
    ...
    existing_neighbours_set = set((asn, neighbour, date.strftime('%Y-%m-%d'), type) for asn, neighbour, date, type in existing_neighbours_result)

    # Filter out neighbours that already exist
    new_neighbours_to_insert = [
        item for item in neighbours
        if (item['asn_req'], item['asn'], item['date'], item['type']) not in existing_neighbours_set
    ]
    ...
    for item in new_neighbours_to_insert:
        values_list.append(f"({item['asn_req']}, {item['asn']}, '{item['date']}', '{item['type']}', {item['power']}, {item['v4_peers']}, {item['v6_peers']})")
    sql += ",\n".join(values_list) + ";"
...
def insert_traffic_for_country_to_db(country_iso2, traffic, save_sql_to_file=False, load_to_database=True):
    ...
    existing_traffic_result = c.execute(existing_traffic_query, {'country_iso2': country_iso2}).fetchall()
    ...
    for item in new_traffic_to_insert:
        values_list.append(f"('{country_iso2}', '{item['timestamp']}', {item['value']})")
    sql += ",\n".join(values_list) + ";"
...
def insert_internet_quality_for_country_to_db(country_iso2, internet_quality, save_sql_to_file=False, load_to_database=True):
    ...
    existing_quality_result = c.execute(existing_quality_query, {'country_iso2': country_iso2}).fetchall()
    ...
    for timestamp, p75, p50, p25 in (
        ...
        if item_timestamp_formatted not in existing_quality_set:
            new_quality_to_insert.append({'timestamp': timestamp, 'p75': p75, 'p50': p50, 'p25': p25})
    ...
    for item in new_quality_to_insert:
        values_list.append(f"('{country_iso2}', '{item['timestamp']}', {item['p75']}, {item['p50']}, {item['p25']})")
    sql += ",\n".join(values_list) + ";"
